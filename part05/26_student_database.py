...
def print_student(database: dict, student: str):
    ...
    # default is zero, when student is added but no courses completed
    ...
    if student in database:
        ...
        if database[student] == "":
            ...
            # if no courses, assisnt with the default value and print
            print(" " + str(database[student]))

        else:
            # if more than one course, print amount of courses
            if len(database[student]) > 0:
                average = 0
                print(
                    " " + str(len(database[student])) + " completed courses:")

                for key, value in database[student]:
                    # print course name and grade obtained
                    print(f"  {key} {value}")
                    average += value
                if average > 0:
