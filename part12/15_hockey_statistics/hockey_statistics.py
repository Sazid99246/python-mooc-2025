...
class HockeyApplication:
    ...
    def add_players(self, all_players: list):
        for player in all_players:
            p = Player(player['name'], player['nationality'], player['assists'],
                       player['goals'], player['penalties'], player['team'], player['games'])
    ...
    def list_players_in_team(self):
        ...
        team_players = sorted(
            team_players, key=self.__points_scored, reverse=True)
        for player in team_players:
            print(player)

    def list_players_in_country(self):
        country = input('country: ')
        country_players = filter(
            lambda player: player.nationality == country, self.players)
        country_players = sorted(
            country_players, key=self.__points_scored, reverse=True)
        for player in country_players:
            print(player)

    def list_players_most_points(self):
        max_num_of_players = int(input('how many: '))
        points_players = sorted(
            self.players, key=self.__points_scored, reverse=True)
        count = 1
    ...
    def list_players_most_goals(self):
        max_num_of_players = int(input('how many: '))
        points_players = sorted(
            self.players, key=self.__goals_scored, reverse=True)
        count = 1
        for player in points_players:
            if count <= max_num_of_players:
                print(player)
